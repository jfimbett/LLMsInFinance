...
- Named Entity Recognition (NER) is a core Natural Language Processing (NLP) task that scans text and tags strings that refer to “real-world” objects—things with a proper name.
In practice, an NER system takes raw sentences and returns spans of text labeled with their semantic category, such as PERSON, LOCATION, ORGANIZATION, DATE, MONEY, and so on.
- Remember when we talked about training? There are specific datasets for NER on which many of these models are trained. 
- E.g. https://www.clips.uantwerpen.be/conll2003/ner/
