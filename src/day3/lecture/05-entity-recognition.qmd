```quarto
// filepath: c:\Users\jimbet\Dropbox\Teaching\LLM\LLMsInFinance\src\day3\lecture\05-entity-recognition.qmd
...
::: {.column width="40%"}
```python
def extract_entity_network(text):
    """
    ...
    Returns:
    Entity network with relationship types
    """
    # Extract entities
    ...
    for e1 in entities:
        for e2 in entities:
            if e1 != e2:
                relation = identify_relationship(text, e1, e2)
                if relation:
...
::: {.column width="40%"}
```python
def build_financial_knowledge_graph(documents):
    """
    Construct knowledge graph from documents
    
    Parameters:
    documents - Collection of financial documents
    ...
    for doc in documents:
        ...
        extraction = extract_entity_network(doc.text)
        
        # Add to graph with document as source
        for entity in extraction["entities"]:
            add_or_update_node(graph, entity, doc)
            
        for rel in extraction["relationships"]:
            add_or_update_edge(graph, rel, doc)
