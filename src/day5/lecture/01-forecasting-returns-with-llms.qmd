```quarto
// filepath: c:\Users\jimbet\Dropbox\Teaching\LLM\LLMsInFinance\src\day5\lecture\01-forecasting-returns-with-llms.qmd
---
...
# Load pre-trained financial LLM
model_name = "financial-bert-large"
tokenizer = AutoTokenizer.from_pretrained(model_name)
model = AutoModelForCausalLM.from_pretrained(model_name)

# Prepare earnings call transcript
transcript = "In Q3, we saw robust growth in our..."

# Create prediction prompt
prompt = f"""
Based on the following earnings call transcript, 
predict the stock's return over the next month. 
...
::: {.column width="40%"}
```
Example prompt for return forecasting:

Analyze the following earnings call transcript 
for Company XYZ and predict the stock's return 
...
4. Analyst question topics and management responses
5. Comparison to previous quarterly language
6. Capex and R&D investment plans
7. Competitive positioning statements

Transcript: [...]

First, extract key points from each consideration.
Then, synthesize these insights to generate a
